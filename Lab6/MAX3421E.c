...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	u8 wdata[2] = {reg+2, val};

	Status = XSpi_Transfer(&SpiInstance, wdata, wdata, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, wdata, wdata, nbytes+1);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	Status = XSpi_Transfer(&SpiInstance, rdata, rdata, 2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, rdata, rdata, nbytes+1);
